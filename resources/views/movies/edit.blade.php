  ...
  <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col my-2">
    <div class="-mx-3 md:flex mb-6">
      <div class="md:w-full px-3">
        ...
        <input name="title" value="{{$movie->title}}" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4 mb-3" id="grid-title" type="text">
      </div>
      <div class="md:w-1/3 px-3 mb-6 md:mb-0">
        <label class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2" for="grid-realisateur">
          Année de sortie
        </label>
        <input name="release_date" value="{{$movie->release_date}}" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4" id="grid-realisateur" type="text" placeholder="2023">
      </div>
    </div>
    <div class="-mx-3 md:flex mb-6">
      <div class="md:w-1/2 px-3 mb-6 md:mb-0">
        ...
        <input name="director" value="{{$movie->director}}" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4" id="grid-realisateur" type="text" placeholder="Albuquerque">
      </div>
        ...
        <div class="relative">
          <select name="category" class="block appearance-none w-full bg-grey-lighter border border-grey-lighter text-grey-darker py-3 px-4 pr-8 rounded" id="grid-state">
            @foreach ($categories as $category)
                <option value="{{$category->id}}"  @if($category->id == $movie->category)selected="selected"@endif>{{$category->name}}</option>
            @endforeach
          </select>
    ...
    <div class="-mx-3 md:flex mb-6">
      <div class="md:w-full px-3">
        ...
        <textarea name="description" rows="10" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded py-3 px-4 mb-3" id="grid-description">{{$movie->description}}</textarea>
    ...
    <div class="-mx-3 md:flex mb-6">
      <div class="md:w-full px-3">
        ...
        <input-file-component image-url="{{$movie->image}}"></input-file-component>
        <p class="text-grey-dark text-xs italic">{{$movie->image}}</p>
      </div>
    </div>
